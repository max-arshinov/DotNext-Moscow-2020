...
namespace Infrastructure.SwaggerSchema.Filters
{
    public class SchemaOperationFilter : IOperationFilter
    {
        ...
        private void AddAdditionalAttributesToSchemas(OpenApiOperation operation,
            List<ControllerParameterDescriptor> list)
        {
            ...
            foreach (var item in list)
            {
                ...
                if (item.ParameterInfo.CustomAttributes.Any(x => x.AttributeType.Name == "FromFormAttribute"))
                {
                    ...
                    foreach (var prop in listOfProperties)
                    {
                        ...
                        if (prop.GetCustomAttribute(typeof(HiddenInputAttribute)) is HiddenInputAttribute
                            hiddenInputAttribute)
                        {
                            operation.RequestBody.Content.FirstOrDefault(x => x.Key == "multipart/form-data")
                                .Value.Schema.Properties.FirstOrDefault(x => x.Key == prop.Name)
                                .Value.Extensions
                                .Add("isHidden", new OpenApiBoolean(hiddenInputAttribute.DisplayValue));
                        }

                        if (prop.PropertyType.IsArray
                            || prop.PropertyType.IsEnum
                            && prop.PropertyType.GetCustomAttribute(typeof(FlagsAttribute)) != null)
                        {
                            operation.RequestBody.Content.FirstOrDefault(x => x.Key == "multipart/form-data")
                                .Value.Schema.Properties.FirstOrDefault(x => x.Key == prop.Name)
                        ...
                        )
                        {
                            ...
                            valueExtensions.Add("mediaPropertyName",
                                new OpenApiString(mediaDataAttribute.MediaTypePropertyName));
                        }
                        ...
                        )
                        {
                            ...
                            valueExtensions.Add("multipleFilesLoading",
                                new OpenApiBoolean(mediaTypeAttribute.MultipleFilesLoading));
                            valueExtensions.Add("maxFileSizeInMb",
                                new OpenApiInteger(mediaTypeAttribute.MaxFileSizeInMb));
                            ...
                            {
                                valueExtensions.Add("mediaDataPropertyName",
                                    new OpenApiString(mediaTypeAttribute.MediaDataPropertyName));
                            }
                        }
                    }
                }
